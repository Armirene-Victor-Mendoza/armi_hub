...
class OrdersLocalDataSource {
  ...
  Future<Database> get database async {
    ...
    _database = await openDatabase(
      ...
      onCreate: (db, version) async {
        ...
        await db.execute(
          'CREATE INDEX idx_scanned_orders_created_at ON $tableName(created_at)',
        );
        await db.execute(
          'CREATE INDEX idx_scanned_orders_status ON $tableName(status)',
        );
        await db.execute(
          'CREATE INDEX idx_scanned_orders_business_store ON $tableName(business_id, store_id)',
        );
      },
      onUpgrade: (db, oldVersion, newVersion) async {
        if (oldVersion < 2) {
          await db.execute(
            "ALTER TABLE $tableName ADD COLUMN city TEXT NOT NULL DEFAULT ''",
          );
        }
        if (oldVersion < 3) {
        ...
        if (oldVersion < 4) {
          await db.execute(
            "ALTER TABLE $tableName ADD COLUMN business_name TEXT",
          );
          await db.execute("ALTER TABLE $tableName ADD COLUMN store_name TEXT");
        }
        if (oldVersion < 5) {
          await db.execute(
            "ALTER TABLE $tableName ADD COLUMN public_order_id TEXT",
          );
          await db.execute(
            "ALTER TABLE $tableName ADD COLUMN business_order_id TEXT",
          );
          await db.execute(
            "ALTER TABLE $tableName ADD COLUMN backend_status TEXT",
          );
        }
        if (oldVersion < 6) {
          await db.execute(
  ...
  Future<void> _ensureRequiredColumns(Database db) async {
    ...
    await _ensureColumn(db, columnName: 'public_order_id', definition: 'TEXT');
    await _ensureColumn(
      db,
      columnName: 'business_order_id',
      definition: 'TEXT',
    );
